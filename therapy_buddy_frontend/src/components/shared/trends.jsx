...
const METRIC_COLORS = { mood: "#5a845f", stress: "#845a5a", sleep: "#5a6c84" };
...
export default function Trends() {
  ...
  const METRICS = [
    { key: "mood",   label: t("mood"),   color: METRIC_COLORS.mood },
    { key: "stress", label: t("stress"), color: METRIC_COLORS.stress },
    { key: "sleep",  label: t("sleep"),  color: METRIC_COLORS.sleep },
  ];
  ...
  const chartData = data?.daily_breakdown?.map(d => ({
    ...
    mood: d.mood, stress: d.stress, sleep: d.sleep,
  })) || [];
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-6 py-8">
      <h2 className="font-display text-2xl mb-1">{t("trends_title")}</h2>
      <p className="text-stone-400 text-sm font-body mb-6">{t("wellness_patterns")}</p>

      <div className="grid grid-cols-2 gap-3 mb-6">
        {score && (
          <div className="bg-white border border-cream-200 rounded-2xl p-4">
            <p className="text-xs font-mono text-stone-400 uppercase tracking-widest mb-2">{t("stability")}</p>
        ...
        {streaks && (
          <div className="bg-white border border-cream-200 rounded-2xl p-4">
            <p className="text-xs font-mono text-stone-400 uppercase tracking-widest mb-2">{t("streaks")}</p>
        ...
        {loading ? (
          <div className="w-full h-full flex items-center justify-center">
            <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }}
              className="text-stone-300 font-body text-sm">{t("loading")}</motion.div>
          </div>
        ) : chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0ebe0" />
              <XAxis dataKey="date" tick={{ fontSize: 11, fill: "#a09080", fontFamily: "DM Mono" }} axisLine={false} tickLine={false} />
              <YAxis domain={[0, 12]} tick={{ fontSize: 11, fill: "#a09080", fontFamily: "DM Mono" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ fontFamily: "DM Sans", fontSize: 13, borderRadius: 12, border: "1px solid #e8e0cc" }}
                cursor={{ stroke: activeColor, strokeWidth: 1, strokeDasharray: "4 4" }} />
              <Line type="monotone" dataKey={activeMetric} stroke={activeColor} strokeWidth={3}
          ...
          <div className="w-full h-full flex flex-col items-center justify-center">
            <p className="font-display text-xl text-stone-300 mb-2">{t("no_data")}</p>
            <p className="text-stone-400 text-sm font-body">{t("start_logging")}</p>
          </div>
        )}
      </div>
      ...
      {data?.averages && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: t("avg_mood"),   value: data.averages.mood },
            { label: t("avg_sleep"),  value: data.averages.sleep + "h" },
            { label: t("avg_stress"), value: data.averages.stress },
          ].map(s => (
